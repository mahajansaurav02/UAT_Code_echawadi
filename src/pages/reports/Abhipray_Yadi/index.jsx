...
const AbhiprayYadi = () => {
    ...
    sendRequest(
      ...
      (res) => {
        ...
        setMessages((prevMessages) => {
          // This stops the chat from reloading or flickering visually.
          ...
          return data.map((item, index) => {
            ...
            return {
              ...
              date: item.remarkDate
                ? moment(item.remarkDate).format('DD MMM YYYY')
                : moment().format('DD MMM YYYY'),
              time: item.remarkTime
                ? moment(item.remarkTime, 'HH:mm:ss').format('h:mm A')
      ...
      (err) => {
        ...
        message.error(
          intl.formatMessage({
            id: 'abhiprayYadi.fetchError',
            defaultMessage: 'Failed to load remarks data.',
          }),
        );
        setLoading(false);
      },
  ...
  // @modification: Use a ref to keep track of waiting status inside the setInterval closure
  const isWaitingRef = useRef(isWaitingForReply);
  useEffect(() => {
    isWaitingRef.current = isWaitingForReply;
  }, [isWaitingForReply]);
  ...
  useEffect(() => {
    ...
    // @modification: Ultra-Smart Polling
    // Only hits the API if the Talathi is actually waiting for a reply from the IO.
    const intervalId = setInterval(() => {
      if (codeVillage && isWaitingRef.current) {
        fetchRemarks();
      }
    }, 30000); // 30 seconds interval is safe for servers

    // Cleanup interval on unmount
    return () => clearInterval(intervalId);
  }, [fetchRemarks, codeVillage]);
  ...
  const handleSubmitReply = () => {
    if (!replyText.trim()) {
      message.warning(
        intl.formatMessage({
          id: 'abhiprayYadi.emptyWarning',
          defaultMessage: 'Please type a remark.',
        }),
      );
      return;
    }
    ...
    sendRequest(
      ...
      (res) => {
        ...
        message.success(
          intl.formatMessage({
            id: 'abhiprayYadi.successMsg',
            defaultMessage: 'Remark submitted successfully!',
          }),
        );

        // @modification: Optimistic UI Update
        const newMessage = {
          id: Date.now(),
        ...
        // Append new message to current chat state
        setMessages((prev) => [...prev, newMessage]);

        // (Optional) We can quietly fetch the latest state from DB to sync up
        fetchRemarks();
      },
      (err) => {
        ...
        >
          ...
          <div className="chatContainer" style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
            {!codeVillage ? (
              ...
              <div style={{ textAlign: 'center', padding: '50px', color: '#999' }}>
                <FormattedMessage
                  id="abhiprayYadi.noRemarks"
                  defaultMessage="No conversation history found."
                />
              </div>
            ) : (
                  ...
                  {groupedMessages[date].map((msg) => (
                    <div key={msg.id} className={`messageRow ${msg.align}`}>
                      ...
                      {msg.align === 'left' && (
                        <Avatar
                          size={40}
                          style={{ backgroundColor: msg.avatarColor, marginTop: '20px' }}
                        >
                          {msg.avatarInitials}
                        </Avatar>
                      )}
                      ...
                      <div className={`messageContent ${msg.align}`}>
                        ...
                        <div className={`messageBubble ${msg.align}`}>{msg.text}</div>

                        {/* Attachments rendering */}
                        {msg.attachment && <div className="attachmentTag">📎 {msg.attachment}</div>}
          ...
          <div
            ...
            style={{
              ...
              backgroundColor: isWaitingForReply ? '#fafafa' : '#fff',
            }}
          >
            ...
            <TextArea
              ...
              placeholder={
                isWaitingForReply
                  ? intl.formatMessage({
                      id: 'abhiprayYadi.waitingForIO',
                      defaultMessage: 'Waiting for Inspection Officer reply...',
                    })
                  : intl.formatMessage({
                      id: 'abhiprayYadi.placeholder',
